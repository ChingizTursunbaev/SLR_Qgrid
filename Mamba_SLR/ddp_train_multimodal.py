#!/usr/bin/env python
# ddp_train_multimodal.py — stable fp32/bf16 + CTC, length guards, LR warmup+cosine
...
def materialize_params_with_dummy_forward(model: nn.Module, loader: DataLoader, device: torch.device, use_bf16: bool):
    ...
    ctx = torch.amp.autocast("cuda", dtype=torch.bfloat16) if use_bf16 else torch.amp.autocast("cuda", enabled=False)
...
def train(args):
    ...
    # discourage blank early so it doesn't dominate
    with torch.no_grad():
        if hasattr(base_model, "classifier") and base_model.classifier.bias is not None:
            base_model.classifier.bias.zero_()
            base_model.classifier.bias[0].fill_(-1.0)  # blank id = 0
    ...
    for epoch in range(args.epochs):
        ...
        for it, batch in enumerate(train_loader):
            ...
            if not torch.isfinite(logits).all():
                ...
                # debug visibility
                if is_main() and (it % (max(1, args.log_interval) * 5) == 0):
                    max_tgt = int(label_lengths.max().item())
                    print(f"[dbg] T_out={T_out}  max_target_len={max_tgt}", flush=True)

                # CTC lengths
                input_lengths_full = torch.full((B,), T_out, dtype=torch.long, device=device)

                # filter invalid samples where target_len > T_out
                valid_mask = (label_lengths <= input_lengths_full) & (label_lengths > 0)
                n_valid = int(valid_mask.sum().item())
                n_drop  = int((~valid_mask).sum().item())
                if n_drop > 0 and is_main() and (it % max(1, args.log_interval) == 0):
                    print(f"[ctc] dropping {n_drop}/{B} samples this step (target_len > T_out)", flush=True)

                if n_valid == 0:
                    (logits.sum() * 0 / accum).backward()
                else:
                    logits_v        = logits[valid_mask]                   # (Bv,T,V)
                    label_lengths_v = label_lengths[valid_mask]            # (Bv,)
                    input_lengths_v = input_lengths_full[valid_mask]       # (Bv,)
                    labels_v        = labels[valid_mask]                   # (Bv,Lmax)

                    # CTC expects (T,B,C) log-probs
                    log_probs = F.log_softmax(logits_v.float(), dim=-1).transpose(0, 1)  # (T,Bv,V)

                    # flatten targets
                    targets_1d = torch.cat(
                        [labels_v[i, : label_lengths_v[i]] for i in range(len(label_lengths_v))],
                        dim=0
                    )

                    # if any blank token slipped into targets, drop those items
                    if (targets_1d == blank_idx).any():
                        if is_main():
                            print("[warn] target sequence contains blank id 0; check your gloss_dict / dataset padding.", flush=True)
                        ok = torch.ones(len(label_lengths_v), dtype=torch.bool, device=device)
                        for i in range(len(label_lengths_v)):
                            if (labels_v[i, : label_lengths_v[i]] == blank_idx).any():
                                ok[i] = False
                        if ok.any():
                            log_probs       = log_probs[:, ok]
                            input_lengths_v = input_lengths_v[ok]
                            kept = [labels_v[i, : label_lengths_v[i]] for i in range(len(ok)) if ok[i]]
                            targets_1d = torch.cat(kept, dim=0)
                        else:
                            (logits.sum() * 0 / accum).backward()
                            # skip to optimizer step bookkeeping
                    ...
                    loss = ctc_loss(log_probs, targets_1d, input_lengths_v, label_lengths_v)
                    if torch.isfinite(loss):
                        (loss / accum).backward()
                        running += float(loss.item()) * n_valid
                        seen    += n_valid
                    else:
                        (logits.sum() * 0 / accum).backward()
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                label_lengths_v = label_lengths[valid_mask]
                input_lengths_v = input_lengths_full[valid_mask]
                labels_v        = labels[valid_mask]

                log_probs = F.log_softmax(logits_v.float(), dim=-1).transpose(0, 1)
                targets_1d = torch.cat(
                    [labels_v[i, : label_lengths_v[i]] for i in range(len(label_lengths_v))],
                    dim=0
                )
                if (targets_1d == 0).any():
                    ok = torch.ones(len(label_lengths_v), dtype=torch.bool, device=device)
                    for i in range(len(label_lengths_v)):
                        if (labels_v[i, : label_lengths_v[i]] == 0).any():
                            ok[i] = False
                    if not ok.any():
                        continue
                    log_probs       = log_probs[:, ok]
                ...
                loss = ctc_loss(log_probs, targets_1d, input_lengths_v, label_lengths_v)
                if torch.isfinite(loss):
                    val_loss += float(loss.item()) * int(valid_mask.sum().item())
                    val_seen += int(valid_mask.sum().item())
...
# ###############################################################################################################################################
#########################################   Very high validation and test loss   ##############################################################
###############################################################################################################################################
# #!/usr/bin/env python
